...
%               segments numerically. pchip = piecewise cubic hermite
...
seglen = sqrt(sum(diff(data,[],1).^2,2));%finding hypotenuse?
arclen = sum(seglen);
...
for i = 1:nd
  switch method
    ...
    case 'spline'
      spl{i} = spline([0;cumsum(chordlen)],data(:,i)); %cumsum = cumulative sum
